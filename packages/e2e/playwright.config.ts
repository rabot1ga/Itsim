...
export default defineConfig({
  ...
  use: {
    ...
    /**
     * Действия без лимита (значение по умолчанию — 0) означают «ждать, пока не
     * кончится тест». Именно это делало «флапы» непереносимыми: клик,
     * перехваченный карточкой сюжета, висел 80+ с и уносил весь тест вместо
     * того, чтобы упасть через 15 с — а на таком упадке `act()`
     * (tests/helpers/story.ts) закрывает карточку и повторяет шаг.
     */
