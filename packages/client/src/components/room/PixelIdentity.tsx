  ...
  return (
    <div className="game-card">
      <div className="flex items-baseline justify-between mb-2">
        ...
        <span className="text-[10px] font-mono text-slate-500">32×32 · {rows.length} слоёв</span>
      </div>

      <div className="grid grid-cols-[96px_1fr] gap-3">
        <PixelAvatar data={data} scale={6} className="rounded-lg border border-slate-700" background="#12141a" />
        <dl className="space-y-1 text-xs">
          {rows.map(({ cat, label, id }) => (
            <div key={cat} className="flex items-center justify-between gap-2">
              <dt className="text-slate-500">{cat}</dt>
              <dd className="flex items-center gap-2">
                <span className="text-slate-200">{label}</span>
                <span
                  className={`text-[9px] px-1 rounded ${
                    data.source[cat] === 'trait' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-700/60 text-slate-400'
                  }`}
                  title={data.source[cat] === 'trait' ? 'взято из генотипа игрока' : 'вытянуто из seed'}
                >
                  {data.source[cat] === 'trait' ? 'генотип' : 'seed'}
                </span>
                <code className="text-[9px] text-slate-600">{id}</code>
              </dd>
            </div>
          ))}
          <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-800">
            <dt className="text-slate-500">палитра</dt>
            <dd className="text-slate-200">{data.scheme?.name ?? 'базовая'}</dd>
          </div>
        </dl>
      </div>
      ...
      {siblingSeeds.length > 0 && (
        <div className="mt-3 flex items-center gap-2">
          <span className="text-[10px] text-slate-500">соседи по seed:</span>
      ...
      <p className="mt-2 text-[10px] font-mono text-slate-600 break-all" title={data.seed}>
