...
> Изометрия — это «мир» игры (комната, офис). Хром поверх него —
...
один спрайт-силуэт (`iso/palette.ts`, `TOP_STYLE`). Карточка шеринга — тоже
теперь плоский стек: `room/ShareCard.tsx` берёт слои из `room/shareScene.ts`,
...
четырёх поверхностях сразу, потому что фильтр считается в одном месте
(`buildLayerStack`). Ряды «цвет верха/низа/обуви» iso-специфичны: у плоских
слоёв одежды нет `tintSlot`,
который складывает их ровно тем же `buildLayerStack` и с теми же долями кадра,
что и `RoomRenderer` (карточка обязана совпадать с игрой кадр в кадр, это и был
единственный смысл её существовать). Удалено в связи с этим: React-компонент
`IsoRoom` и iso-пейнтер `iso/canvas.ts` (`drawIsoRoom`).

Не удалено, но и не вызывается из приложения: генератор iso-комнаты
(`iso/scene.ts` → `buildRoomScene`, `roomSeed`) с правилами «уровень жилья →
мебель, питомец с миской, ничего не залезает друг в друга». Он по-прежнему
полностью покрыт тестами (`iso/__tests__/scenes.test.ts`, 11 проверок) и делит
аллокатор с офисом, поэтому оставлен как есть; решение «вернуть iso-вид
альтернативой или вычистить вместе с тестами» — открытое, см.
[TODO.md](TODO.md) §2. Ниже — как устроен конвейер и где что править.
