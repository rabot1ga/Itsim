...
Один и тот же движок рисует офис компании (`IsoOffice`) и карточку шеринга
(`ShareCard` → `canvas.drawIsoRoom`); список спрайтов и перекраску дают
`iso/manifest.ts` (`useIsoManifest`, `useSpriteVariants`).
...
один спрайт-силуэт (`iso/palette.ts`, `TOP_STYLE`). Сам iso-рендер комнаты
(`IsoRoom`) удалён как мёртвый: mount-точек не осталось, спрайты при этом
нужны офису, HUD-портрету и шерингу. Ниже — как устроен конвейер и где что
править.
