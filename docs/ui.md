...
Вкладка «Навыки» рисует ветки как **RPG-деревья**, а не списки: `skills.json`
и так был деревом (`parent` + `unlockAt` у каждого навыка) — теперь форма
видна. Раскладку считает `layoutBranch()` в `screens/skillTreeLayout.ts` из
контента: колонка — на каждый «лист» ветки, родитель центрируется над
потомками, ряды — глубины; рёбра рисуются SVG-полилиниями. Новые ветки
...
Перекрёстные требования (`ai_ml → backend/python`, `blockchain →
frontend/javascript`) ведут себя как «чужая школа»: нода — корень своей
ветки, но с замком и подписью, пока родитель из другой ветки не прокачан.

Тест геометрии: `src/screens/__tests__/skillsTree.test.ts`.
