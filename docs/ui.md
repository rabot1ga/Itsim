...
### 8.3. Дерево навыков — одна карта

Вкладка «Навыки» — **одно дерево всей игры**, Path-of-Exile-стайл: никаких
подразделов-веток. `skills.json` — лес связанных деревьев: школы требуют друг
друга (AI/ML висит на backend-`python`, blockchain — на frontend-`javascript`,
кибербез — на devops-`linux`), поэтому весь граф раскладывается на **один холст**
и показывается картой, которую можно тащить и зумить.

Раскладка — `screens/skillTreeLayout.ts`:

- `layoutBranch()` — одна связная «роща»: колонка на каждый лист, родитель
  центрируется над потомками, ряды — глубины, рёбра SVG-полилиниями;
- `skillComponents()` + `layoutForest()` — делят контент на связные компоненты
  по рёбрам `parent` и пакуют их в сбалансированные колонки одной карты.
...
`skills.json`: каждый навык ровно один раз, рощи не пересекаются, родитель
внутри веера потомков).
