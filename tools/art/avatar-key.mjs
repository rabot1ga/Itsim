 ...
 *
 * Клиент не масштабирует слои: каждый файл уже лежит на канвасе CW×CH, и
 * `buildLayerStack()` просто stacked их по zOrder. Значит вся посадка — на
 * совести OVERRIDES, и менять её нужно вместе с ассетами, прогоняя
 * `npm run avatar:check -w tools/art` (см. avatar-v2/README.md).
 *
 * Использование (обычно не напрямую, а через avatar-v2/build.mjs):
 *   node tools/art/avatar-key.mjs body  <master.png>            <out.webp>
 *   node tools/art/avatar-key.mjs layer <master.png> <slot> <out.webp>
 * `slot` пока только логируется: режим выбирается по имени файла в OVERRIDES.
 */
...
const OVERRIDES = {
  ...
  'hair_short.png': { mode: 'anchor', scale: 0.1, ax: 0.5, ay: 0.18, rx: 250, ry: L.crown + 18 },
  'hair_short_v2.png': { mode: 'anchor', scale: 0.1, ax: 0.5, ay: 0.18, rx: 250, ry: L.crown + 18 },
  'hair_short_v3.png': { mode: 'anchor', scale: 0.1, ax: 0.5, ay: 0.18, rx: 250, ry: L.crown + 18 },
  ...
  'hair_long.png': { mode: 'anchor', scale: 0.16, ax: 0.5, ay: 0.1, rx: 250, ry: L.crown + 5 },
  ...
  'beard_full.png': { mode: 'anchor', targetW: 90, targetH: 85, ax: 0.5, ay: 0.2, rx: 250, ry: L.mouth + 5 },
  'beard_mustache.png': { mode: 'anchor', targetW: 70, targetH: 18, ax: 0.5, ay: 0.3, rx: 250, ry: L.nose + 12 },
  // ---- tops ----
  ...
  // ---- accessories ----
  'acc_cap.png': { mode: 'anchor', targetW: 140, targetH: 80, ax: 0.5, ay: 0.7, rx: 250, ry: L.crown + 25 },
  'acc_headphones.png': { mode: 'anchor', targetW: 200, targetH: 120, ax: 0.5, ay: 0.6, rx: 250, ry: L.ear + 5 },
  'acc_beanie.png': { mode: 'anchor', targetW: 130, targetH: 100, ax: 0.5, ay: 0.7, rx: 250, ry: L.crown + 25 },
...
function pupils(mask, data, w, h, silh) {
  ...
  if (!best) return null;
  return {
    lx: Math.min(best.a.cx, best.b.cx),
    rx: Math.max(best.a.cx, best.b.cx),
    y: (best.a.cy + best.b.cy) / 2,
    span: Math.abs(best.a.cx - best.b.cx),
  };
}
...
function topBlockBbox(mask, w, h, silh, yRange, minGap = 8) {
  ...
  let started = false,
    gapRun = 0;
  const minStart = silh.y + Math.floor(silh.h * 0.1);
...
function midBlockBbox(mask, w, h, silh, yRange, minGap = 6) {
  ...
  let beardTop = minY,
    gapRun = 0,
    started = false;
  const minStart = silh.y + Math.floor(silh.h * (yRange[0] + (yRange[1] - yRange[0]) * 0.15));
...
async function placeAnchor(bufP, bw, bh, scale, ax, ay, rx, ry) {
  ...
  let tw = Math.max(1, Math.round(bw * scale)),
    th = Math.max(1, Math.round(bh * scale));
...
async function buildLayer(inp, slot, out) {
  ...
  if (s.w <= 0 || s.h <= 0) {
    ...
    console.log(JSON.stringify({ out, empty: true }));
    return;
  }
  ...
  if (ov?.mode === 'pupil') {
    ...
    const c2 = new Uint8Array(fresh.data.length);
    c2.set(fresh.data);
    ...
    const eyeSpan = ov.eyeSpan || 80,
      eyeY = ov.eyeY || L.eye;
    // rx/ry — куда на канвасе сажать середину глаз; sc — масштаб из разлёда зрачков.
    // Якорь внутри спрайта считается ниже, по bbox «раздетого» слоя.
    const rx = 250,
      ry = eyeY;
    const sc = pp ? eyeSpan / pp.span : 0.14;
    ...
    const stripCopy = new Uint8Array(fresh.data.length);
    stripCopy.set(fresh.data);
    ...
    for (let p = 0; p < m2.length; p++) if (!m2[p]) fresh.data[p * 4 + 3] = 0;
    const hm = keepBig(
      denoise(coloredPix(m2, c2, fresh.info.width, fresh.info.height, s, [0, 0.62], 20)),
      fresh.info.width,
      fresh.info.height,
      0.03
    );
    ...
    for (let p = 0; p < hm.length; p++) {
      if (!hm[p]) {
        fresh.data[p * 4] = fresh.data[p * 4 + 1] = fresh.data[p * 4 + 2] = fresh.data[p * 4 + 3] = 0;
        continue;
      }
      const y = Math.floor(p / fresh.info.width);
      if (y < hb.y || y >= hb.y + hb.h)
        fresh.data[p * 4] = fresh.data[p * 4 + 1] = fresh.data[p * 4 + 2] = fresh.data[p * 4 + 3] = 0;
    }
    ...
    const bm = keepBig(
      denoise(coloredPix(m2, c2, fresh.info.width, fresh.info.height, s, [0.3, 0.85], 30)),
      fresh.info.width,
      fresh.info.height,
      0.03
    );
    const bb = midBlockBbox(bm, fresh.info.width, fresh.info.height, s, [0.3, 0.85]);
    ...
    const m2 = keepBig(denoise(keyMg(c2, fresh.info.width, fresh.info.height)), fresh.info.width, fresh.info.height);
    for (let p = 0; p < m2.length; p++) if (!m2[p]) fresh.data[p * 4 + 3] = 0;
    const W = fresh.info.width,
      H = fresh.info.height;
    // Torso zone: from ~22% below top of silhouette (armpits down). Keeps shirts/jackets
    // and avoids capturing hair/face.
    const sm2 = keepBig(denoise(coloredPix(m2, c2, W, H, s, [0.22, 1.0])), W, H, 0.1);
    for (let p = 0; p < sm2.length; p++)
      if (!sm2[p]) fresh.data[p * 4] = fresh.data[p * 4 + 1] = fresh.data[p * 4 + 2] = fresh.data[p * 4 + 3] = 0;
    ...
    const W = fresh.info.width,
      H = fresh.info.height;
    const bm2 = keepBig(denoise(coloredPix(m2, c2, W, H, s, [0.38, 1.0])), W, H, 0.1);
    for (let p = 0; p < bm2.length; p++)
      if (!bm2[p]) fresh.data[p * 4] = fresh.data[p * 4 + 1] = fresh.data[p * 4 + 2] = fresh.data[p * 4 + 3] = 0;
    ...
    for (let p = 0; p < m2.length; p++) if (!m2[p]) fresh.data[p * 4 + 3] = 0;
    const sk = keepBig(
      skinPix(m2, fresh.data, fresh.info.width, fresh.info.height),
      fresh.info.width,
      fresh.info.height,
      0.05
    );
    ...
    for (let p = 0; p < m2.length; p++) if (!m2[p]) fresh.data[p * 4 + 3] = 0;
    let mm = keepBig(
      denoise(mannequinPix(m2, fresh.data, fresh.info.width, fresh.info.height), fresh.info.width, fresh.info.height),
      fresh.info.width,
      fresh.info.height
    );
    ...
    if (mbox.w <= 0) {
      mm = m2;
      mbox = bbox(m2, fresh.info.width, fresh.info.height);
    }
    // find max-width row in top 30% of mannequin bbox (shoulders)
    let sw = 0,
      sy = mbox.y,
      scx = mbox.x + mbox.w / 2;
    for (let y = mbox.y; y < mbox.y + Math.floor(mbox.h * 0.3); y++) {
    ...
    let cw = Infinity,
      cy = s.y,
      ccx = s.x + s.w / 2;
    for (let y = s.y + Math.floor(s.h * 0.3); y < s.y + Math.floor(s.h * 0.7); y++) {
    ...
    // Like item but uses coloredPix (colored non-skin, non-grey) to isolate accessory.
    const fresh = await loadRaw(inp);
    const c2 = new Uint8Array(fresh.data.length);
    c2.set(fresh.data);
    const m2 = keepBig(denoise(keyMg(c2, fresh.info.width, fresh.info.height)), fresh.info.width, fresh.info.height);
    ...
    const it = keepBig(denoise(it0, fresh.info.width, fresh.info.height), fresh.info.width, fresh.info.height, 0.05);
    ...
    if (ib.w <= 0) {
      ...
      for (let p = 0; p < useMask.length; p++)
        if (!useMask[p]) useData[p * 4] = useData[p * 4 + 1] = useData[p * 4 + 2] = useData[p * 4 + 3] = 0;
  ...
  const final = blank().composite([{ input: placed.rz, left: placed.left, top: placed.top }]);
  await mkdir(dirname(out), { recursive: true });
  await final.webp({ quality: 92 }).toFile(out);
  console.log(JSON.stringify({ out, s }));
}

// Модуль импортируют (QA-страж), поэтому CLI живёт только при прямом запуске.
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  ...
  if (cmd === 'body') {
    ...
    const [i, s, o] = args;
    await buildLayer(i, s, o);
  } else {
    console.error('bad cmd', cmd);
    console.error('usage: avatar-key.mjs body <master> <out> | layer <master> <slot> <out>');
    process.exit(1);
  }
}
